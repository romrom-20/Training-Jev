...
TEXT_VARIANTS = {
    "food": (
        ("The food was delicious and fresh.", "The meal tasted excellent and hearty."),
        ("The food was bland and stale.", "The meal tasted awful and cold."),
    ),
    "service": (
        ("The staff were friendly and attentive.", "The service was warm and helpful."),
        ("The staff were rude and inattentive.", "The service was cold and unhelpful."),
    ),
    "value": (
        ("The price felt fair and reasonable.", "It was good value for the money."),
    ...
    for split, count in SPLITS.items():
        for group_no in range(count):
            ...
            for bits in itertools.product((0, 1), repeat=3):
                ...
                clauses = [
                    TEXT_VARIANTS[aspect][1 - bits[j]][variant] for j, aspect in enumerate(ASPECTS)
                ]
                for fmt in range(4) if split == "shift_test" else (0,):
                    ...
                    for active, aspect in enumerate(ASPECTS):
                        ...
                        rows.append(
                            {
                                "id": f"{group}-b{''.join(map(str, bits))}-q{active}-f{fmt}",
                                "group": group,
                                "split": split,
                                "group_no": group_no,
                                "bits": list(bits),
...
def collect(run, offline):
    ...
    try:
        with torch.inference_mode():
            for offset in range(0, len(rows), 8):
                ...
                if offset % 192 == 0:
                    print(
                        f"captured {min(offset + len(batch_rows), len(rows))}/{len(rows)}",
                        flush=True,
                    )
    finally:
    ...
    for i, row in enumerate(rows):
        word = (
            generated[i].strip().split(maxsplit=1)[0].strip(".,:;!?\"'()[]{}").lower()
            if generated[i].strip()
            else ""
        )
    ...
    np.savez_compressed(
        ...
        h=np.concatenate(hs),
        q=q.numpy(),
        conditional_p_positive=probs[:, 0],
        label_mass=masses,
    )
    write_json(run / "dataset.json", rows)
    capability = []
    ...
    write_json(
        run / "manifest.json",
        {
            ...
            "protocol_sha256": hashlib.sha256(
                Path("docs/experiments/007-aspect-sentiment-readout.md").read_bytes()
            ).hexdigest(),
...
def pack(h, q, rows, indices):
    return (
        h[indices],
        q[[rows[i]["active"] for i in indices]],
...
def train(run):
    ...
    for li, layer in enumerate(LAYERS):
        train_ix = np.array(
            [i for i, r in enumerate(rows) if r["split"] == "train" and r["format"] == 0]
        )
        val_ix = np.array(
            [i for i, r in enumerate(rows) if r["split"] == "validation" and r["format"] == 0]
        )
        cal_ix = np.array(
            [i for i, r in enumerate(rows) if r["split"] == "calibration" and r["format"] == 0]
        )
        ...
        for seed, method in itertools.product((0, 1, 2), ("bilinear", "independent")):
            ...
            probe = (
                ...
                if method == "bilinear"
                else IndependentProbe(h.shape[-1], tasks=3)
            )
            fitted = fit_probe(
                probe, train_data, val_data, steps=300, lr=0.01, weight_decay=0.01, brier_weight=0.1
            )
            with torch.no_grad():
                    ...
                    summaries.append(
                        {
                            **key,
                            ...
                            "calibrated": metrics(
                                y[mask], sigmoid(eval_logits[mask] / temperature)
                            ),
                        }
                    )
    ...
    text_model.fit(
        [rows[i]["user"] for i in train_text_ix], [rows[i]["label"] for i in train_text_ix]
    )
    text_p = text_model.predict_proba([r["user"] for r in test_rows])[
        :, list(text_model.classes_).index(1)
    ]
    text_metrics = []
    for fmt in (0, 1, 2, 3):
        for aspect in range(3):
            ...
            text_metrics.append(
                {
                    "format": fmt,
                    "aspect": ASPECTS[aspect],
                    "metrics": metrics(y[mask], text_p[mask]),
                }
            )
    ...
    primary_index = [
        i for i, k in enumerate(keys) if k["layer"] == 16 and k["method"] == "bilinear"
    ]
    primary = (
        logits_saved[primary_index]
        / np.asarray([keys[i]["temperature"] for i in primary_index])[:, None]
    )
    ...
    for fmt in (1, 2, 3):
        for aspect in range(3):
            anchor = (
                (formats == fmt)
                & (active == aspect)
                & np.isin([r["group_no"] for r in test_rows], range(8))
            )
            source_anchor = (
                (formats == 0)
                & (active == aspect)
            ...
            eval_mask = (
                (formats == fmt)
                & (active == aspect)
                & np.isin([r["group_no"] for r in test_rows], range(8, 24))
            )
            ...
            corrections.append(
                {
                    ...
                    "offset": offset,
                    "baseline": metrics(yy, p_base),
                    "centered": metrics(yy, p_shift),
                    "delta_brier_centered_minus_base": delta,
