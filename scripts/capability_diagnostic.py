    ...
    for review_id, (review, label) in enumerate(REVIEWS):
        for phrasing, prompt in enumerate(PROMPTS):
            for mapping in (1, -1):
                ...
                text = text.replace("A means positive; B means negative.", f"{positive} means positive; {negative} means negative.")
                text = text.replace("Positive = A. Negative = B.", f"Positive = {positive}. Negative = {negative}.")
                text = text.replace("A: positive sentiment", f"{positive}: positive sentiment").replace("B: negative sentiment", f"{negative}: negative sentiment")
                text = text.replace("Code A = positive; code B = negative.", f"Code {positive} = positive; code {negative} = negative.")
                result.append({"id": f"r{review_id}-p{phrasing}-m{mapping}", "review": review, "label": label, "phrasing": phrasing, "mapping": mapping, "answer": positive if label else negative, "user": text})
    return result
...
def run(model_config, output_root, offline):
    ...
    chats = [tokenizer.apply_chat_template([{"role": "user", "content": row["user"]}], tokenize=False, add_generation_prompt=True) for row in examples]
    ...
    for offset in range(0, len(examples), 8):
        ...
        tokens = tokenizer(batch_text, padding=True, return_tensors="pt", truncation=False).to(device)
        ...
        with torch.inference_mode():
            ...
            generated = model.generate(**tokens, do_sample=False, max_new_tokens=4, pad_token_id=tokenizer.pad_token_id)
        ...
        for i, row in enumerate(batch_rows):
            ...
            predictions.append({**row, "response": response, "first_token_text": first,
                                "strict_correct": first == row["answer"],
                                "conditional_correct": ("A" if conditional[i, 0] >= conditional[i, 1] else "B") == row["answer"],
                                "conditional_p_a": float(conditional[i, 0]), "ab_mass": float(mass[i])})
        print(f"{model_config['name']}: {min(offset + len(batch_rows), len(examples))}/{len(examples)}", flush=True)
    cells = []
    for phrasing, mapping in itertools.product(range(4), (1, -1)):
        ...
        cells.append({"phrasing": phrasing, "mapping": mapping, "n": len(selected),
    ...
    result = {"model": model_config, "device": device, "created_at": datetime.now(timezone.utc).isoformat(),
              "seconds": time.perf_counter() - started, "n": len(predictions), "cells": cells,
              "pooled_strict_accuracy": float(np.mean([r["strict_correct"] for r in predictions])),
              "pooled_conditional_accuracy": float(np.mean([r["conditional_correct"] for r in predictions])),
              "capability_gate": all(c["strict_accuracy"] >= 0.9 for c in cells),
              "protocol_sha256": hashlib.sha256(Path("docs/experiments/005-capability-diagnostic.md").read_bytes()).hexdigest(),
              "rows": predictions}
...
def main():
    ...
    parser.add_argument("--models", nargs="+", choices=("qwen-0.5b", "qwen-1.5b"), default=("qwen-0.5b", "qwen-1.5b"))
    args = parser.parse_args()
    if args.output.exists():
        raise FileExistsError(f"Refusing to overwrite {args.output}")
    args.output.mkdir(parents=True)
    all_models = {
        "qwen-0.5b": {"name": "qwen-0.5b", "model": "Qwen/Qwen2.5-0.5B-Instruct", "revision": "7ae557604adf67be50417f59c2c2f167def9a775"},
        "qwen-1.5b": {"name": "qwen-1.5b", "model": "Qwen/Qwen2.5-1.5B-Instruct", "revision": "989aa7980e4cf806f80c7fef2b1adb7bc71aa306"},
    }
