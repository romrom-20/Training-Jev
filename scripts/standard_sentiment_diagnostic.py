...
def run(model_config, output_root, offline):
    ...
    rows = [{"id": f"review-{i}-format-{fmt}", "review": review, "label": label,
             "format": fmt, "answer": "positive" if label else "negative",
             "user": PROMPTS[fmt].format(review=review)}
            for i, (review, label) in enumerate(REVIEWS) for fmt in range(4)]
    ...
    label_ids = [tokenizer.encode(label, add_special_tokens=False) for label in ("positive", "negative")]
    ...
    for offset in range(0, len(rows), 8):
        ...
        decoded = tokenizer.batch_decode(generated[:, tokens.input_ids.shape[1]:], skip_special_tokens=True)
        for i, row in enumerate(batch):
            response = decoded[i]
            first_word = response.strip().split(maxsplit=1)[0].strip(".,:;!?\"'()[]{}").lower() if response.strip() else ""
            predictions.append({**row, "response": response, "first_word": first_word,
                                "strict_correct": first_word == row["answer"],
                                "conditional_correct": ("positive" if conditional[i, 0] >= conditional[i, 1] else "negative") == row["answer"],
                                "conditional_p_positive": float(conditional[i, 0]),
                                "label_mass": float(mass[i])})
        print(f"{model_config['name']}: {min(offset + len(batch), len(rows))}/{len(rows)}", flush=True)
    cells = []
    for fmt in range(4):
        selected = [r for r in predictions if r["format"] == fmt]
        cells.append({"format": fmt, "n": len(selected),
    ...
    result = {"model": model_config, "device": device, "created_at": datetime.now(timezone.utc).isoformat(),
              ...
              "protocol_sha256": hashlib.sha256(Path("docs/experiments/006-standard-sentiment-capability.md").read_bytes()).hexdigest(),
              "rows": predictions}
